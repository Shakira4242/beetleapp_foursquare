...
import {
  ...
  delay,
  all,
  cancelled as cancelled3,
  cancel
} from "@redux-saga/core/effects";

// src/redux/features/session/sessionSaga.ts
import { eventChannel as eventChannel2 } from "@redux-saga/core";
import {
  call as call2,
  put,
  take,
...
import { take as take2, cancelled as cancelled2 } from "@redux-saga/core/effects";
...
function* initSessionSaga({
  ...
  yield fork3(sessionChannelWatcher, {
    sessionChannel,
    pubSubChannel,
    swEventChannel
  });
  const sessionStatusTask = yield fork3(sessionStatusWatcher, {
    session,
    sessionChannel,
    pubSubChannel,
    userOptions
...
function* socketClosedWorker({
  ...
  if (session.status === "reconnecting") {
    yield put3(pubSubChannel, sessionReconnectingAction());
    yield delay(Math.random() * 2e3);
    yield call3(session.connect);
  } else if (session.status === "disconnected") {
    yield put3(pubSubChannel, sessionDisconnectedAction());
    sessionChannel.close();
  } else {
...
function* reauthenticateWorker({
  ...
  try {
    if (session.reauthenticate) {
      ...
      yield put3(pubSubChannel, sessionConnectedAction());
    }
  } catch (error) {
    getLogger().error("Reauthenticate Error", error);
    yield put3(authErrorAction({ error }));
  }
}
    ...
    while (true) {
      ...
      switch (action.type) {
        case authSuccessAction.type: {
          ...
          startSagaTask = yield fork3(startSaga, options);
          break;
        }
        case authErrorAction.type: {
          yield fork3(sessionAuthErrorSaga, __spreadProps(__spreadValues({}, options), {
            action
          }));
          break;
        }
        case authExpiringAction.type: {
          yield put3(options.pubSubChannel, sessionExpiringAction());
          break;
        }
        case socketErrorAction.type:
          break;
        case socketClosedAction.type:
          yield fork3(socketClosedWorker, options);
          break;
        case reauthAction.type: {
          yield fork3(reauthenticateWorker, {
...
function* startSaga(options) {
  ...
  try {
    ...
    const pubSubTask = yield fork3(pubSubSaga, {
      pubSubChannel,
      emitter: userOptions.emitter
    });
    const executeActionTask = yield fork3(executeActionWatcher, session);
    yield put3(sessionActions.connected(session.rpcConnectResult));
    yield put3(pubSubChannel, sessionConnectedAction());
    const flushExecuteQueueTask = yield fork3(flushExecuteQueueWorker);
...
function* sessionAuthErrorSaga(options) {
  ...
  try {
    ...
    pubSubTask = yield fork3(pubSubSaga, {
      pubSubChannel,
      emitter: userOptions.emitter
    });
    yield put3(pubSubChannel, sessionAuthErrorAction(error));
    sessionChannel.close();
  } finally {
    if (yield cancelled3()) {
...
var rootSaga_default = (options) => {
  return function* root({
    ...
    yield fork3(executeQueueWatcher);
...
import { fork as fork4, put as put4 } from "@redux-saga/core/effects";
...
function* memberUpdatedWorker({
  ...
  for (const key of updated) {
    ...
    yield put4(channels.pubSubChannel, {
      type,
...
var memberPositionWorker = function* memberPositionWorker2({
  ...
  while (true) {
    ...
    switch (action.type) {
      case "video.member.updated": {
        yield fork4(memberUpdatedWorker, {
          action,
          channels,
          memberList,
          instance
      ...
      case "video.layout.changed": {
        yield fork4(memberPositionLayoutChangedWorker, {
          action,
          channels,
          memberList,
          instance
...
import { multicastChannel as multicastChannel2 } from "@redux-saga/core";
...
var createPubSubChannel = () => multicastChannel2();
