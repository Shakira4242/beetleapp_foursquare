...
import { fork as fork4, call as call3, take as take4, put as put4, delay as delay2, all } from "@redux-saga/core/effects";
...
function* executeActionWatcher(session) {
  function* worker(action) {
    ...
    try {
      ...
      const response = yield call2(session.execute, message);
      if (componentId && requestId) {
        yield put(componentActions.executeSuccess({
          componentId,
          requestId,
          response
        }));
      ...
      if (componentId && requestId) {
        yield put(componentActions.executeFailure({
          componentId,
          requestId,
          action,
          error
...
import { take as take2 } from "@redux-saga/core/effects";
...
function* pubSubSaga({
  ...
  while (true) {
    const pubSubAction = yield take2(pubSubChannel, "*");
...
import { put as put2, take as take3, fork as fork2, select as select2 } from "@redux-saga/core/effects";
...
function* executeQueueWatcher() {
  function* worker(action) {
    const authStatus = yield select2(getAuthStatus);
    if (authStatus !== "authorized") {
      yield put2(executeQueueActions.add(action.payload));
    }
  }
  while (true) {
    const action = yield take3(executeAction.type);
    yield fork2(worker, action);
  }
}
function* flushExecuteQueueWorker() {
  const { queue } = yield select2(getExecuteQueue);
  for (const params of queue) {
    yield put2(executeAction(params));
  }
  yield put2(executeQueueActions.clean());
}
...
import { delay, fork as fork3, put as put3, select as select3 } from "@redux-saga/core/effects";
function* componentCleanupSagaWorker() {
  const toCleanup = yield select3(getComponentsToCleanup);
  if (toCleanup.length) {
    yield put3(componentActions.cleanup({
      ids: toCleanup
    }));
  }
}
function* componentCleanupSaga() {
  while (true) {
    yield delay(36e5);
    yield fork3(componentCleanupSagaWorker);
  }
}
...
function* initSessionSaga({
  ...
  const sessionChannel = yield call3(createSessionChannel, session);
  ...
  if ((_a = userOptions.workers) == null ? void 0 : _a.length) {
    try {
      const effects = userOptions.workers.map((saga) => {
        return call3(createRestartableSaga(saga));
      });
  ...
  yield fork4(sessionChannelWatcher, {
    session,
    sessionChannel,
    pubSubChannel,
    swEventChannel
  });
  yield fork4(sessionStatusWatcher, {
    session,
    sessionChannel,
    pubSubChannel,
    userOptions
  });
  const compCleanupTask = yield fork4(componentCleanupSaga);
  session.connect();
  yield take4(destroyAction.type);
...
function* socketClosedWorker({
  ...
  if (session.status === "reconnecting") {
    yield put4(pubSubChannel, sessionReconnectingAction());
    yield delay2(Math.random() * 2e3);
    yield call3(session.connect);
  } else if (session.status === "disconnected") {
    yield put4(pubSubChannel, sessionDisconnectedAction());
    sessionChannel.close();
  } else {
...
function* reauthenticateWorker({
  ...
  try {
    if (session.reauthenticate) {
      session.token = token;
      yield call3(session.reauthenticate);
      yield put4(pubSubChannel, sessionConnectedAction());
    }
  } catch (error) {
    getLogger().error("Reauthenticate Error", error);
    yield put4(authErrorAction({ error }));
  }
}
function* sessionStatusWatcher(options) {
  while (true) {
    const action = yield take4([
    ...
    switch (action.type) {
      case authSuccessAction.type:
        yield fork4(startSaga, options);
        break;
      case authErrorAction.type: {
        yield fork4(sessionAuthErrorSaga, {
          action,
      ...
      case authExpiringAction.type: {
        yield put4(options.pubSubChannel, sessionExpiringAction());
        break;
      }
      case socketErrorAction.type:
        break;
      case socketClosedAction.type:
        yield fork4(socketClosedWorker, options);
        break;
      case reauthAction.type: {
        yield fork4(reauthenticateWorker, {
...
function* startSaga(options) {
  ...
  const pubSubTask = yield fork4(pubSubSaga, {
    pubSubChannel,
    emitter: userOptions.emitter
  });
  const executeActionTask = yield fork4(executeActionWatcher, session);
  yield put4(sessionActions.connected(session.rpcConnectResult));
  yield put4(pubSubChannel, sessionConnectedAction());
  const flushExecuteQueueTask = yield fork4(flushExecuteQueueWorker);
  yield take4(closeConnectionAction.type);
  pubSubTask.cancel();
...
function* sessionAuthErrorSaga(options) {
  ...
  yield put4(pubSubChannel, sessionAuthErrorAction(error));
  yield take4(sessionDisconnectedAction);
  pubSubTask.cancel();
  throw error;
}
var rootSaga_default = (options) => {
  return function* root({
    ...
    yield fork4(executeQueueWatcher);
    yield take4(initAction.type);
    try {
      yield call3(initSessionSaga, __spreadProps(__spreadValues({}, options), {
        userOptions,
        channels
      }));
    } catch (error) {
...
var configureStore2 = (options) => {
  ...
  const pubSubChannel = multicastChannel();
...
import { fork as fork5, put as put5 } from "@redux-saga/core/effects";
...
function* memberUpdatedWorker({
  ...
  for (const key of updated) {
    ...
    yield put5(channels.pubSubChannel, {
      type,
...
var memberPositionWorker = function* memberPositionWorker2({
  ...
  while (true) {
    ...
    switch (action.type) {
      case "video.member.updated": {
        yield fork5(memberUpdatedWorker, {
          action,
          channels,
          memberList,
          instance
      ...
      case "video.layout.changed": {
        yield fork5(memberPositionLayoutChangedWorker, {
          action,
          channels,
          memberList,
          instance
