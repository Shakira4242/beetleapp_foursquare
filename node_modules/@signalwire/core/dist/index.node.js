...
var BaseSession = class {
  ...
  _onSocketClose(event) {
    ...
    this._status = event.code == 1e3 ? "disconnected" : "reconnecting";
...
var findNamespaceInPayload = (action) => {
  ...
  if (action.payload === void 0) {
    return "";
  } else if (isVideoMemberEvent(action) || isVideoLayoutEvent(action) || isVideoRecordingEvent(action) || isVideoPlaybackEvent(action) || isVideoRoomAudienceCountEvent(action)) {
...
function* initSessionSaga({
  ...
  const sessionChannel = yield (0, import_effects5.call)(createSessionChannel, session);
  ...
  if ((_a = userOptions.workers) == null ? void 0 : _a.length) {
    try {
      const effects = userOptions.workers.map((saga) => {
        return (0, import_effects5.call)(createRestartableSaga(saga));
      });
      customTasks = yield (0, import_effects5.all)(effects);
    } catch (error) {
      getLogger().error("Error running custom workers", error);
    }
  }
  yield (0, import_effects5.fork)(sessionChannelWatcher, {
    sessionChannel,
    pubSubChannel,
    swEventChannel
  });
  const sessionStatusTask = yield (0, import_effects5.fork)(sessionStatusWatcher, {
    session,
    sessionChannel,
    pubSubChannel,
    userOptions
  ...
  session.connect();
  yield (0, import_effects5.take)(destroyAction.type);
  yield (0, import_effects5.delay)(300);
...
function* socketClosedWorker({
  ...
  if (session.status === "reconnecting") {
    yield (0, import_effects5.put)(pubSubChannel, sessionReconnectingAction());
    yield (0, import_effects5.delay)(Math.random() * 2e3);
    yield (0, import_effects5.call)(session.connect);
  } else if (session.status === "disconnected") {
    yield (0, import_effects5.put)(pubSubChannel, sessionDisconnectedAction());
    sessionChannel.close();
  } else {
...
function* reauthenticateWorker({
  ...
  try {
    if (session.reauthenticate) {
      session.token = token;
      yield (0, import_effects5.call)(session.reauthenticate);
      yield (0, import_effects5.put)(pubSubChannel, sessionConnectedAction());
    }
  } catch (error) {
    getLogger().error("Reauthenticate Error", error);
    yield (0, import_effects5.put)(authErrorAction({ error }));
  }
}
function* sessionStatusWatcher(options) {
  getLogger().debug("sessionStatusWatcher [started]");
  let startSagaTask = void 0;
  try {
    while (true) {
      const action = yield (0, import_effects5.take)([
      ...
      switch (action.type) {
        case authSuccessAction.type: {
          if (startSagaTask) {
            yield (0, import_effects5.cancel)(startSagaTask);
          }
          startSagaTask = yield (0, import_effects5.fork)(startSaga, options);
          break;
        }
        case authErrorAction.type: {
          yield (0, import_effects5.fork)(sessionAuthErrorSaga, __spreadProps(__spreadValues({}, options), {
            action
          }));
          break;
        }
        case authExpiringAction.type: {
          yield (0, import_effects5.put)(options.pubSubChannel, sessionExpiringAction());
          break;
        }
        case socketErrorAction.type:
          break;
        case socketClosedAction.type:
          yield (0, import_effects5.fork)(socketClosedWorker, options);
          break;
        case reauthAction.type: {
          yield (0, import_effects5.fork)(reauthenticateWorker, {
    ...
    if (yield (0, import_effects5.cancelled)()) {
...
function* startSaga(options) {
  ...
  try {
    ...
    const pubSubTask = yield (0, import_effects5.fork)(pubSubSaga, {
      pubSubChannel,
      emitter: userOptions.emitter
    });
    const executeActionTask = yield (0, import_effects5.fork)(executeActionWatcher, session);
    yield (0, import_effects5.put)(sessionActions.connected(session.rpcConnectResult));
    yield (0, import_effects5.put)(pubSubChannel, sessionConnectedAction());
    const flushExecuteQueueTask = yield (0, import_effects5.fork)(flushExecuteQueueWorker);
    yield (0, import_effects5.take)(closeConnectionAction.type);
    pubSubTask.cancel();
...
function* sessionAuthErrorSaga(options) {
  ...
  try {
    ...
    pubSubTask = yield (0, import_effects5.fork)(pubSubSaga, {
      pubSubChannel,
      emitter: userOptions.emitter
    });
    yield (0, import_effects5.put)(pubSubChannel, sessionAuthErrorAction(error));
    sessionChannel.close();
  } finally {
    if (yield (0, import_effects5.cancelled)()) {
      getLogger().debug("sessionAuthErrorSaga [cancelled]", pubSubTask == null ? void 0 : pubSubTask.isCancelled());
    }
    yield (0, import_effects5.delay)(10);
    yield (0, import_effects5.put)(destroyAction());
  }
}
var rootSaga_default = (options) => {
  return function* root({
    ...
    yield (0, import_effects5.fork)(executeQueueWatcher);
    yield (0, import_effects5.take)(initAction.type);
    try {
      yield (0, import_effects5.call)(initSessionSaga, __spreadProps(__spreadValues({}, options), {
        userOptions,
        channels
      }));
    } catch (error) {
...
var createMemberPromoteDemoteMethod = (method) => {
  return {
...
var promote = createMemberPromoteDemoteMethod("video.member.promote");
var demote = createMemberPromoteDemoteMethod("video.member.demote");
...
function* memberPositionLayoutChangedWorker(options) {
  ...
  for (const [memberId, payload] of memberList) {
    if (processedMembers[memberId]) {
      yield (0, import_effects6.put)(pubSubChannel, {
...
function* memberUpdatedWorker({
  ...
  for (const key of updated) {
    ...
    yield (0, import_effects6.put)(channels.pubSubChannel, {
      type,
...
var memberPositionWorker = function* memberPositionWorker2({
  ...
  while (true) {
    ...
    switch (action.type) {
      case "video.member.updated": {
        yield (0, import_effects6.fork)(memberUpdatedWorker, {
          action,
          channels,
          memberList,
          instance
      ...
      case "video.layout.changed": {
        yield (0, import_effects6.fork)(memberPositionLayoutChangedWorker, {
          action,
          channels,
          memberList,
          instance
