...
var toSnakeCaseKeys = (obj, transform = (value) => value, result = {}) => {
  if (Array.isArray(obj)) {
    result = obj.map((item, index) => toSnakeCaseKeys(item, transform, result[index]));
  } else {
...
var validateEventsToSubscribe = (events) => {
  const valid = events.map((internalEvent) => {
    if (typeof internalEvent === "string") {
      ...
      if (CLIENT_SIDE_EVENT_NAMES.includes(event) || isSyntheticEvent(event) || isSessionEvent(event)) {
        return null;
      }
...
__export(actions_exports, {
  ...
  socketClosedAction: () => socketClosedAction,
...
var closeConnectionAction = createAction("swSdk/closeConnection");
...
var socketClosedAction = createAction("socket/closed");
var socketErrorAction = createAction("socket/error");
...
var BaseSession = class {
  ...
  connect() {
    ...
    const openHandler = (event) => {
      var _a;
      (_a = this._socket) == null ? void 0 : _a.removeEventListener("open", openHandler);
      this._onSocketOpen(event);
    };
    this._socket.addEventListener("open", openHandler);
    const closeHandler = (event) => {
      var _a;
      (_a = this._socket) == null ? void 0 : _a.removeEventListener("close", closeHandler);
      this._onSocketClose(event);
    };
    this._socket.addEventListener("close", closeHandler);
    const errorHandler = (event) => {
      var _a;
      (_a = this._socket) == null ? void 0 : _a.removeEventListener("error", errorHandler);
  ...
  execute(msg) {
    if (this._status === "idle" || !this.connected) {
      return Promise.reject("Can't call `execute` when Session is not authorized.");
    }
    let promise;
    if ("params" in msg) {
  ...
  _onSocketClose(event) {
    ...
    this._status = event.code >= 1006 && event.code <= 1014 ? "reconnecting" : "disconnected";
...
var import_core2 = __toESM(require("@redux-saga/core"));
...
var import_effects6 = require("@redux-saga/core/effects");
...
function* executeActionWatcher(session) {
  function* worker(action) {
    const authStatus = yield (0, import_effects2.select)(getAuthStatus);
...
function* sessionChannelWatcher({
  ...
  function* vertoWorker({ jsonrpc, nodeId }) {
    var _a, _b;
    ...
    switch (method) {
      case "verto.media": {
        ...
        yield (0, import_effects2.put)(componentActions.upsert(component));
        yield (0, import_effects2.put)(executeAction({
      ...
      case "verto.answer": {
        ...
        if (params == null ? void 0 : params.sdp) {
      ...
      case "verto.bye": {
        const component = {
          ...
          state: "hangup",
          nodeId,
          byeCause: (_a = params == null ? void 0 : params.cause) != null ? _a : "",
          byeCauseCode: (_b = params == null ? void 0 : params.causeCode) != null ? _b : 0,
          redirectDestination: params == null ? void 0 : params.redirectDestination
        };
        yield (0, import_effects2.put)(componentActions.upsert(component));
        yield (0, import_effects2.put)(executeAction({
          method: "video.message",
      ...
      case "verto.mediaParams": {
        ...
        if (mediaParams == null ? void 0 : mediaParams.video) {
          component.videoConstraints = mediaParams.video;
        }
        if (mediaParams == null ? void 0 : mediaParams.audio) {
  ...
  function* videoAPIWorker(params) {
    switch (params.event_type) {
      ...
      case "video.room.subscribed": {
        yield (0, import_effects2.put)(componentActions.upsert({
      ...
      case "video.member.joined": {
        ...
        if (member == null ? void 0 : member.parent_id) {
          const parent = yield (0, import_effects2.select)(getComponent, member.parent_id);
          if (parent) {
            yield (0, import_effects2.put)(componentActions.upsert({
              id: member.id,
  ...
  function* swEventWorker(broadcastParams) {
    ...
    if (isWebrtcEvent(broadcastParams)) {
      yield (0, import_effects2.fork)(vertoWorker, {
...
function createSessionChannel(session) {
  return (0, import_core.eventChannel)((emit) => {
...
var findNamespaceInPayload = (action) => {
  ...
  if (action.payload === void 0) {
    return "";
  } else if (isVideoMemberEvent(action) || isVideoLayoutEvent(action) || isVideoRecordingEvent(action) || isVideoPlaybackEvent(action)) {
...
// src/redux/features/executeQueue/executeQueueSaga.ts
var import_effects4 = require("@redux-saga/core/effects");

// src/redux/features/executeQueue/executeQueueSelectors.ts
var getExecuteQueue = ({ executeQueue }) => {
  return executeQueue;
};

// src/redux/features/executeQueue/executeQueueSaga.ts
function* executeQueueWatcher() {
  function* worker(action) {
    const authStatus = yield (0, import_effects4.select)(getAuthStatus);
    if (authStatus !== "authorized") {
      yield (0, import_effects4.put)(executeQueueActions.add(action.payload));
    }
  }
  while (true) {
    const action = yield (0, import_effects4.take)(executeAction.type);
    yield (0, import_effects4.fork)(worker, action);
  }
}
function* flushExecuteQueueWorker() {
  const { queue } = yield (0, import_effects4.select)(getExecuteQueue);
  for (const params of queue) {
    yield (0, import_effects4.put)(executeAction(params));
  }
  yield (0, import_effects4.put)(executeQueueActions.clean());
}
...
// src/redux/features/component/componentSaga.ts
var import_effects5 = require("@redux-saga/core/effects");
function* componentCleanupSagaWorker() {
  const toCleanup = yield (0, import_effects5.select)(getComponentsToCleanup);
  if (toCleanup.length) {
    yield (0, import_effects5.put)(componentActions.cleanup({
      ids: toCleanup
    }));
  }
}
function* componentCleanupSaga() {
  while (true) {
    yield (0, import_effects5.delay)(36e5);
    yield (0, import_effects5.fork)(componentCleanupSagaWorker);
  }
}
...
function* initSessionSaga({
  ...
  const sessionChannel = yield (0, import_effects6.call)(createSessionChannel, session);
  ...
  if ((_a = userOptions.workers) == null ? void 0 : _a.length) {
    try {
      const effects = userOptions.workers.map((saga) => {
        return (0, import_effects6.call)(createRestartableSaga(saga));
      });
      customTasks = yield (0, import_effects6.all)(effects);
    } catch (error) {
      getLogger().error("Error running custom workers", error);
    }
  }
  yield (0, import_effects6.fork)(sessionChannelWatcher, {
    session,
    sessionChannel,
    pubSubChannel,
    swEventChannel
  });
  yield (0, import_effects6.fork)(sessionStatusWatcher, {
    session,
    sessionChannel,
    pubSubChannel,
    userOptions
  });
  const compCleanupTask = yield (0, import_effects6.fork)(componentCleanupSaga);
  session.connect();
  yield (0, import_effects6.take)(destroyAction.type);
  compCleanupTask == null ? void 0 : compCleanupTask.cancel();
  pubSubChannel.close();
  sessionChannel.close();
  swEventChannel.close();
  customTasks.forEach((task) => task.cancel());
}
function* socketClosedWorker({
  session,
  sessionChannel,
  pubSubChannel
}) {
  ...
  if (session.status === "reconnecting") {
    yield (0, import_effects6.put)(pubSubChannel, sessionReconnectingAction());
    yield (0, import_effects6.delay)(Math.random() * 2e3);
    yield (0, import_effects6.call)(session.connect);
  } else if (session.status === "disconnected") {
    yield (0, import_effects6.put)(pubSubChannel, sessionDisconnectedAction());
    sessionChannel.close();
  } else {
...
function* reauthenticateWorker({
  ...
  try {
    if (session.reauthenticate) {
      session.token = token;
      yield (0, import_effects6.call)(session.reauthenticate);
      yield (0, import_effects6.put)(pubSubChannel, sessionConnectedAction());
    }
  } catch (error) {
    getLogger().error("Reauthenticate Error", error);
    yield (0, import_effects6.put)(authErrorAction({ error }));
  }
}
function* sessionStatusWatcher(options) {
  while (true) {
    const action = yield (0, import_effects6.take)([
    ...
    switch (action.type) {
      case authSuccessAction.type:
        yield (0, import_effects6.fork)(startSaga, options);
        break;
      case authErrorAction.type: {
        yield (0, import_effects6.fork)(sessionAuthErrorSaga, {
          action,
      ...
      case authExpiringAction.type: {
        yield (0, import_effects6.put)(options.pubSubChannel, sessionExpiringAction());
        break;
      }
      case socketErrorAction.type:
        break;
      case socketClosedAction.type:
        yield (0, import_effects6.fork)(socketClosedWorker, options);
        break;
      case reauthAction.type: {
        yield (0, import_effects6.fork)(reauthenticateWorker, {
...
function* startSaga(options) {
  ...
  const pubSubTask = yield (0, import_effects6.fork)(pubSubSaga, {
    pubSubChannel,
    emitter: userOptions.emitter
  });
  const executeActionTask = yield (0, import_effects6.fork)(executeActionWatcher, session);
  yield (0, import_effects6.put)(sessionActions.connected(session.rpcConnectResult));
  yield (0, import_effects6.put)(pubSubChannel, sessionConnectedAction());
  const flushExecuteQueueTask = yield (0, import_effects6.fork)(flushExecuteQueueWorker);
  yield (0, import_effects6.take)(closeConnectionAction.type);
  pubSubTask.cancel();
...
function* sessionAuthErrorSaga(options) {
  ...
  yield (0, import_effects6.put)(pubSubChannel, sessionAuthErrorAction(error));
  yield (0, import_effects6.take)(sessionDisconnectedAction);
  pubSubTask.cancel();
  throw error;
}
var rootSaga_default = (options) => {
  return function* root({
    ...
    yield (0, import_effects6.fork)(executeQueueWatcher);
    yield (0, import_effects6.take)(initAction.type);
    try {
      yield (0, import_effects6.call)(initSessionSaga, __spreadProps(__spreadValues({}, options), {
        userOptions,
        channels
      }));
    } catch (error) {
...
var configureStore2 = (options) => {
  ...
  const sagaMiddleware = (0, import_core2.default)();
  const pubSubChannel = (0, import_core2.multicastChannel)();
  const swEventChannel = (0, import_core2.multicastChannel)();
  const channels = {
    pubSubChannel,
    swEventChannel
  };
...
var BaseComponent = class {
  constructor(options) {
    ...
    __publicField(this, "uuid", (0, import_uuid.v4)());
    __publicField(this, "_proxyFactoryCache", /* @__PURE__ */ new WeakMap());
  ...
  subscribe() {
    return __async(this, null, function* () {
      ...
      if (subscriptions.length === 0) {
        this.logger.warn("`subscribe()` was called without any listeners attached.");
        return;
      }
      const execParams = {
...
var getRecordings = createRoomMethod("video.recording.list", {
...
var getPlaybacks = createRoomMethod("video.playback.list", {
  transformResolve: (payload) => ({
...
var setMeta = createRoomMethod("video.set_meta", {
  transformResolve: baseCodeTransform,
...
var import_effects7 = require("@redux-saga/core/effects");
function* memberPositionLayoutChangedWorker(options) {
  ...
  layers.forEach((layer) => {
    ...
    if (layer.position !== ((_a = memberEventParams.member) == null ? void 0 : _a.current_position)) {
  ...
  for (const [memberId, payload] of memberList) {
    if (processedMembers[memberId]) {
      yield (0, import_effects7.put)(pubSubChannel, {
...
function* memberUpdatedWorker({
  ...
  for (const key of updated) {
    ...
    yield (0, import_effects7.put)(channels.pubSubChannel, {
      type,
...
var memberPositionWorker = function* memberPositionWorker2({
  ...
  while (true) {
    ...
    switch (action.type) {
      case "video.member.updated": {
        yield (0, import_effects7.fork)(memberUpdatedWorker, {
          action,
          channels,
          memberList,
          instance
      ...
      case "video.layout.changed": {
        yield (0, import_effects7.fork)(memberPositionLayoutChangedWorker, {
          action,
          channels,
          memberList,
          instance
