...
function debounce(fn, wait = 0, callFirst) {
  let timeout = null;
...
// src/video/RoomSession.ts
var RoomSessionConsumer = class extends BaseConsumer {
  constructor(options) {
    super(options);
...
import { getLogger as getLogger2 } from "@signalwire/core";
...
var Session = class extends BaseSession {
  constructor() {
    ...
    __publicField(this, "agent", "@signalwire/realtime-api/3.2.0");
  }
};
...
var VideoClient = function(options) {
  ...
  client.on("session.connected", async () => {
    try {
      ...
      getLogger2().error("Client subscription failed.");
      client.disconnect();
    }
  });
  const interceptors = {
...
var VideoAPI = class extends BaseConsumer2 {
  constructor() {
    super(...arguments);
    __publicField(this, "_eventsPrefix", "video");
    __publicField(this, "subscribeParams", {
...
var taskWorker = function* (options) {
  getLogger3().trace("taskWorker started");
  ...
  getLogger3().trace("taskWorker ended");
};
...
var messagingWorker = function* (options) {
  ...
  while (true) {
    ...
    getLogger4().debug("messagingWorker:", action);
    switch (action.type) {
      ...
      default: {
        getLogger4().warn("[messagingWorker] Unrecognized Action", action);
        break;
      }
    }
  }
...
var voiceCallStateWorker = function* (options) {
  ...
  const { swEventChannel, pubSubChannel } = channels;
  getLogger5().trace("voiceCallStateWorker started", instance.id, instance.tag);
  let run = true;
  ...
  getLogger5().trace("voiceCallStateWorker ended", instance.id, instance.tag);
};
...
var voiceCallReceiveWorker = function* (options) {
  var _a;
  getLogger6().trace("voiceCallReceiveWorker started");
  const { channels, instance } = options;
  const { swEventChannel, pubSubChannel } = channels;
  ...
  getLogger6().trace("voiceCallReceiveWorker ended");
};
...
var voiceCallPlayWorker = function* (options) {
  getLogger7().trace("voiceCallPlayWorker started");
  ...
  getLogger7().trace("voiceCallPlayWorker ended");
};
...
var voiceCallRecordWorker = function* (options) {
  getLogger8().trace("voiceCallRecordWorker started");
  const { channels, instance, initialState } = options;
  const { swEventChannel, pubSubChannel } = channels;
  const { controlId } = initialState;
  if (!controlId) {
  ...
  getLogger8().trace("voiceCallRecordWorker ended");
};
...
var voiceCallPromptWorker = function* (options) {
  getLogger9().trace("voiceCallPromptWorker started");
  const { channels, instance, initialState } = options;
  const { swEventChannel, pubSubChannel } = channels;
  const { controlId } = initialState;
  if (!controlId) {
  ...
  getLogger9().trace("voiceCallPromptWorker ended");
};
...
var voiceCallTapWorker = function* (options) {
  getLogger10().trace("voiceCallTapWorker started");
  const { channels, instance, initialState } = options;
  const { swEventChannel, pubSubChannel } = channels;
  const { controlId } = initialState;
  if (!controlId) {
  ...
  getLogger10().trace("voiceCallTapWorker ended");
};
...
var voiceCallConnectWorker = function* (options) {
  getLogger11().trace("voiceCallConnectWorker started");
  const { channels, instance } = options;
  const { swEventChannel, pubSubChannel } = channels;
  let run = true;
  const done = () => run = false;
  while (run) {
    const action = yield sagaEffects10.take(swEventChannel, (action2) => {
      var _a;
      return action2.type === "calling.call.connect" && (action2.payload.tag === instance.tag || ((_a = action2.payload.peer) == null ? void 0 : _a.tag) === instance.tag);
    });
    const payloadWithTag = __spreadValues({
      tag: instance.tag
    }, action.payload);
  ...
  getLogger11().trace("voiceCallConnectWorker ended");
};
...
var voiceCallDialWorker = function* (options) {
  ...
  const { swEventChannel } = channels;
  getLogger12().trace("voiceCallDialWorker started");
  ...
  getLogger12().trace("voiceCallDialWorker ended");
};
...
var voiceCallSendDigitsWorker = function* (options) {
  getLogger13().trace("voiceCallSendDigitsWorker started");
  ...
  getLogger13().trace("voiceCallSendDigitsWorker ended");
};
...
var voiceCallDetectWorker = function* (options) {
  getLogger14().trace("voiceCallDetectWorker started");
  const { channels, instance, initialState } = options;
  const { swEventChannel, pubSubChannel } = channels;
  ...
  getLogger14().trace("voiceCallDetectWorker ended");
};
...
var CallPromptAPI = class extends AutoApplyTransformsConsumer {
  constructor() {
    super(...arguments);
    __publicField(this, "_eventsPrefix", "calling");
    __publicField(this, "callId");
...
var CallTapAPI = class extends BaseComponent6 {
  constructor() {
    super(...arguments);
    __publicField(this, "callId");
    __publicField(this, "nodeId");
...
var CallDetectAPI = class extends BaseComponent7 {
  constructor() {
    super(...arguments);
    __publicField(this, "_eventsPrefix", "calling");
    __publicField(this, "callId");
      ...
      [
        ["calling.connect.connected"],
        {
          ...
          payloadTransform: (payload) => {
            ...
            return toExternalJSON6(peerParams);
          }
        }
      ],
      [
  ...
  connect(params) {
    return new Promise((resolve, reject) => {
      ...
      const rejectHandler = (payload) => {
        ...
        reject(toExternalJSON6(payload));
      };
    ...
    return /* @__PURE__ */ new Map([
      [
        "calling.call.received",
        {
          ...
          payloadTransform: (payload) => {
            return toExternalJSON7(payload);
          }
        }
      ]
    ]);
